...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  // Explicit focus nodes so we can fully drop focus before opening dialogs.
  final _phoneFocusNode = FocusNode();
  final _firstNameFocusNode = FocusNode();
  final _lastNameFocusNode = FocusNode();
  final _emailFocusNode = FocusNode();
  // A non-input focus node used as a "sink" — focusing it drops keyboard
  // without leaving any text field as the "previously focused" node that
  // Flutter would restore when a dialog closes.
  ...
  Future<void> _openConsentDialog(BuildContext ctx) async {
    // Move focus to the dummy node before opening the dialog.
    // Flutter restores focus to the "previously focused" node when a dialog
    // closes — by making that node a non-input dummy, the keyboard stays down.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        // Tapping anywhere outside a text field moves focus to the dummy node,
        // which has no keyboard — so the keyboard dismisses and won't come back.
        onTap: () => _dummyFocusNode.requestFocus(),
                  ...
                  final rightPanel = PlatformGlassCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_showSuggestions &&
                            customerState.results.isNotEmpty)
